...
class UsersMixin:
    ...
    def get_users_completed_funnel(self):
        """
        Получить пользователей, которые завершили ТЕКУЩУЮ воронку
        
        Критерии завершения воронки:
        1. Получили последнее сообщение воронки (запись в message_deliveries)
        2. НЕТ неотправленных запланированных сообщений (текущая воронка завершена, не перезапущена)
        3. Активны, дали согласие, не оплатили
        
        Это предотвращает ситуацию, когда пользователь:
        - Завершил воронку → отписался → вернулся → воронка перезапустилась
        - В message_deliveries есть старая запись о последнем сообщении
        - Но текущая воронка в процессе (есть неотправленные scheduled_messages)

        Returns:
            List: Список пользователей, завершивших ТЕКУЩУЮ воронку
        """
        conn = self._get_connection()
        cursor = conn.cursor()
        ...
        try:
            ...
            # Получаем пользователей, завершивших ТЕКУЩУЮ воронку:
            # 1. Получили последнее сообщение (есть в message_deliveries)
            # 2. НЕТ неотправленных запланированных сообщений (воронка завершена, не перезапущена)
            cursor.execute('''
            ...
            users = cursor.fetchall()
            logger.info(f"✅ Найдено {len(users)} пользователей, завершивших ТЕКУЩУЮ воронку")
            return users
